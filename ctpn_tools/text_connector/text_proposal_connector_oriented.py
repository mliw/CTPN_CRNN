...
class TextProposalConnectorOriented:
    ...
    def get_text_lines(self, text_proposals, scores, im_size):
        ...
        """
        # Variable tp_groups is very crucial
        # It records different groups of proposals
        # Proposals in the same group can be connected into a single text line
        tp_groups=self.group_text_proposals(text_proposals, scores, im_size) #
        ...
        for index, tp_indices in enumerate(tp_groups):
            ...
            z1 = np.polyfit(X,Y,1)
            x0=np.min(text_line_boxes[:, 0]) # Left side 
            x1=np.max(text_line_boxes[:, 2]) # Right side
            offset=(text_line_boxes[0, 2]-text_line_boxes[0, 0])*0.5 # Half of the proposal width

            # lt_y:left_top_y rt_y:right_top_y
            lt_y, rt_y=self.fit_y(text_line_boxes[:, 0], text_line_boxes[:, 1], x0+offset, x1-offset)
            # lb_y:left_bottom_y rb_y:right_bottom_y            
            lb_y, rb_y=self.fit_y(text_line_boxes[:, 0], text_line_boxes[:, 3], x0+offset, x1-offset)

            score=scores[list(tp_indices)].sum()/float(len(tp_indices)) # Take the average score of all proposals as the score of textline
            text_lines[index, 0]=x0
            text_lines[index, 1]=min(lt_y, rt_y)# top y
            text_lines[index, 2]=x1
            text_lines[index, 3]=max(lb_y, rb_y)# bottom y
